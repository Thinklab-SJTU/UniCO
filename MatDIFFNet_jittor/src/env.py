...
from .utils import to_numpy, check_dim
from ml4co_kit import ATSPSolver, ATSPDataGenerator, TSPSolver


class FakeDataset(jt.dataset.Dataset):
    ...
    def __getitem__(self, idx: int):
        return Var([self.data_size])


class MatDIFFNetEnv(object):
    def __init__(
        self,
    ...
    def _process_data(self, dists: Var, ref_tour: Var) -> Var:
        # check dim
        check_dim(dists, 2)
        ...
        # to Var
        adj_matrix = jt.Var(adj_matrix)
        return adj_matrix

    def process_data(self, dists: Var, ref_tours: Var) -> Var:
        # check dim
        check_dim(dists, 3)
        ...
        # jt.cat
        adj_matrix = jt.concat(adj_matrix_list, dim=0)
        adj_matrix = adj_matrix.reshape(-1, nodes_num, nodes_num)
        return adj_matrix
        
    def generate_data(self, batch_size: int) -> Tuple[Var, Var]:
    ...
    def generate_data_tsp(self, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def generate_data_atsp(self, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def generate_data_sat(self, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def generate_data_hcp(self, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
        tmp_astp_generator = ATSPDataGenerator(
    ...
    def generate_data_vc(self, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
