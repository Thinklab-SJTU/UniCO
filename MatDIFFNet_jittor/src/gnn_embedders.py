...
class PositionEmbeddingSine(nn.Module):
    ...
    def execute(self, node_coords: Var) -> Union[Var, Var]:
        """
        node_coords (B, N, 2)
        ...
        dim_t = 2.0 * jt.divide(dim_t, 2) / self.embedding_dim
        ...
        x_embed = jt.stack((x_embed_0_sin, x_embed_0_cos), dim=3).flatten(2)
        y_embed_0_sin = y_embed[:, :, 0::2].sin()
        y_embed_0_cos = y_embed[:, :, 1::2].cos()
        y_embed = jt.stack((y_embed_0_sin, y_embed_0_cos), dim=3).flatten(2)
...
class ScalarEmbeddingSine3D(nn.Module):
    ...
    def execute(self, egde_weights: Var):
        """
        egde_weights: (B, N, N)
        ...
        dim_t = 2 * jt.divide(dim_t, 2) / self.embedding_dim
        dim_t = self.temperature ** dim_t
        ...
        embed = jt.stack((embed_0_sin, embed_1_cos), dim=4).flatten(3)
        return embed
...
class ScalarEmbeddingSine1D(nn.Module):
    ...
    def execute(self, node_coords: Var):
        ...
        embed_0_sin = embed[:, 0::2].sin()
        embed_1_cos = embed[:, 1::2].cos()
        embed = jt.stack((embed_0_sin, embed_1_cos), dim=2).flatten(1)
        return embed