...
# from jt_sparse import SparseVar
# from jt_sparse import sum as sparse_sum
# from jt_sparse import mean as sparse_mean
# from jt_sparse import max as sparse_max
...
class GNNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm_h1 = {
            ...
            "batch": nn.BatchNorm(hidden_dim, affine=learn_norm) # , track_running_stats=track_norm)
        }.get(self.norm, None)
    ...
    def execute(
        ...
        Args:
            ...
            graph: jt_sparse.SparseVar
            mode: str
            edge_index: Edge indices (2 x E)
            sparse: Whether to use sparse Vars (True/False)
        Returns:
            Updated node and edge features
        """
        # jt.set_printoptions(threshold=numpy.inf)
        if not sparse:
        ...
        Uh1: Var = self.U1(h1)  # B x V x H
        Uh2: Var = self.U2(h2)  # B x V x H
        Vh1: Var = self.V1(h1)  # B x V x H
        Vh2: Var = self.V2(h2)  # B x V x H
        ...
        Ah1: Var = self.A1(h1)  # B x V x H, source
        Ah2: Var = self.A2(h2)  # B x V x H, targetR
        Ce: Var = self.C(e)  # B x V x V x H / E x H
        ...
        # Gates
        gates = jt.sigmoid(e)  # B x V x V x H / E x H
        gates_T = jt.sigmoid(e.transpose(1, 2))
    ...
    def aggregate(
        ...
        Args:
            ...
            graph: jt_sparse.SparseVar (E edges for V x V adjacency matrix)
        ...
        if not sparse:
            if (mode or self.aggregation) == "mean":
                return jt.sum(Vh, dim=2) / (jt.sum(graph, dim=2).unsqueeze(-1).type_as(Vh))
            elif (mode or self.aggregation) == "max":
                return jt.max(Vh, dim=2)[0]
            else:
                return jt.sum(Vh, dim=2)
        # else:
        #     sparseVh = SparseVar(
        #         row=edge_index[0],
        #         col=edge_index[1],
