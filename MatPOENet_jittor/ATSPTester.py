...
from utils_jittor.utils import get_result_folder, AverageMeter, TimeEstimator
...
class ATSPTester:
    def __init__(
        ...
        self.cls_scores = jt.zeros((2, 4)) # atsp, euc, hcp, 3sat
        ...
        # self.all_problems = jt.empty(size=(file_count, node_cnt, node_cnt))
        self.all_problems = []
    ...
    def _test_one_batch(self, idx_start, idx_end):
        ...
        problems_batched = jt.Var(self.all_problems[idx_start: idx_end])
        ...
        with jt.no_grad():
            ...
            max_pomo_reward = aug_reward.max(dim=2)  # get best results from pomo
            # shape: (augmentation, batch)
            no_aug_score = -max_pomo_reward[0, :].float().mean()  # negative sign to make positive value

            max_aug_pomo_reward = max_pomo_reward.max(dim=0)  # get best results from augmentation
            # shape: (batch,)
