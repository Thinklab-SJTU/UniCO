...
def make_positional_encoding_cosh(dim, n_node, scaler=10):
    position_phi = - torch.arange(0, n_node, dtype=torch.float).unsqueeze(1) / n_node 
    dim_phi = torch.arange(0, dim, dtype=torch.float) / dim
    pe = 1 / torch.cosh((position_phi + dim_phi) * scaler)
    return pe

def make_positional_encoding_sin(dim, n_node):
    position_phi = torch.arange(0, n_node, dtype=torch.float).unsqueeze(1) / n_node * 2 * np.pi
    dim_phi = torch.arange(0, dim, dtype=torch.float) / dim * 2 * np.pi
    pe = torch.sin(position_phi + dim_phi)
    return pe

def make_positional_encoding_x1(dim, n_node):
    position_phi = - torch.arange(0, n_node, dtype=torch.float).unsqueeze(1) / n_node 
    dim_phi = torch.arange(0, dim, dtype=torch.float) / dim
    pe = 1 / (torch.abs(position_phi + dim_phi) + 1)
    return pe

def make_positional_encoding_zero(dim, n_node):
    return torch.zeros(n_node, dim)

def make_positional_encoding_origin(dim, n_node):
    position = torch.arange(0, n_node).unsqueeze(1)
    div_term = torch.exp((torch.arange(0, dim, 2, dtype=torch.float) *
                            -(math.log(10000.0) / dim)))
    pe = torch.cat([
        torch.sin(position.float() * div_term),
        torch.cos(position.float() * div_term)
    ], dim=1)
    return pe

def make_positional_encoding(dim, n_node):
    position = torch.arange(0, n_node, dtype=torch.float).unsqueeze(1)
    omega = np.pi * 2 * position / n_node *\
                    torch.cat([
                        torch.pow(2, torch.arange(0, dim/2 - 1, dtype=torch.float)),
                        torch.zeros((1,))])
    s = torch.sin(omega)
    c = torch.cos(omega)
    pe = torch.cat([s, c], dim=1)
    return pe
...
class NormLinear(nn.Module):
    def __init__(self, d_in, d_out, bias=False):
        ...
        self.linear.weight.data /= torch.sum(self.linear.weight.data * self.linear.weight.data)
